...
def start_command(message):
    bot.send_message(
        message.chat.id,
        'Привет! Я могу показать вам курсы обмена. \n' +
...
def help_command(message):
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton(
            'Message the developer', url='telegram.me/s_sitnickiy'
        )
    )
    bot.send_message(
        message.chat.id,
        ...
        '3) Вы получите сообщение, содержащее информацию об исходной и целевой валютах, курсы покупки и продажи. \n' +
...
def get_ex_from_iq_data(ex_json):
    return {
        'buy': ex_json['b'],
        'sale': ex_json['s']
    }
...
def query_text(inline_query):
    bot.answer_inline_query(
        inline_query.id,
        get_iq_articles(pb.get_exchange(inline_query.query))
    )
